...
class MetaAconC_1D(nn.Module):
    ...
    def __init__(self, width, r=16):
        super().__init__()
        self.fc1 = nn.Linear(width, max(r, width // r), bias=True)
        self.bn1 = nn.BatchNorm1d(max(r, width // r))
        self.fc2 = nn.Linear(max(r, width // r), width, bias=True)
    ...
    def forward(self, x):
        beta = torch.sigmoid(
            self.bn2(self.fc2(self.bn1(self.fc1(x)))))
